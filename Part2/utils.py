...
def train(train_loader, model, optimizer, criterion, device, regularizer=None, lr_schedule=None, epoch=None):
    loss_sum = 0.0
    correct = 0.0
    ...
    with tqdm(total=num_iters) as pbar:
        for i, (inp, target) in enumerate(train_loader):
            if lr_schedule is not None:
                try:
                    lr = lr_schedule.triangle_scheduler(i, epoch)
                except:
...
def test(test_loader, model, criterion, device, regularizer=None, **kwargs):
    ...
    with tqdm(total=num_iters) as pbar:
        for input, target in test_loader:
            ...
            logits, nll  = model(input, target, **kwargs)
            loss = nll.clone()
            if regularizer is not None:
                loss += regularizer(model)
...
def predictions(test_loader, model, device, **kwargs):
    ...
    for input, target in test_loader:
        ...
        logits, loss  = model(input, target, **kwargs)
