...
class Triangle(Module):
    ...
    def forward(self, t):
        index = torch.argsort(t).data
        t_coeff = torch.zeros(self.num_points)
        t_coeff[-1] = 3 * t[index[0]]
        t_coeff[2 - index[-1]] = t[index[-1]] - t[index[1]]
        t_coeff[index[0] + 3] = 2 * (t[index[1]] - t[index[0]])
        return t_coeff
    
class TriangleNet(Module):
    def __init__(self, num_classes, architecture, fix_centers=True, architecture_kwargs={}):
        super (TriangleNet, self).__init__()
        self.num_classes = num_classes
        self.dim = 2
        self.fix_centers = fix_centers
        self.fix_points = [True] * (self.dim + 1) + [fix_centers] * (self.dim + 1) + [False]
        self.num_points = len(self.fix_points)
        
        self.simplex = Triangle
        self.architecture = architecture
        
        self.l2 = 0.0
        self.coeff_layer = self.simplex()
        self.net = self.architecture(num_classes, fix_points=self.fix_points, **architecture_kwargs)
        self.curve_modules = []
        for module in self.net.modules():
            if issubclass(module.__class__, CurveModule):
        ...
        if self.dim == 2:
            self.weight_order = {0: 0, 1: 1, 2: 2, (0, 1): 3, (0, 2): 4, (1, 2): 5, (0, 1, 2): 6}
                
    def import_base_parameters(self, base_model, min_index):
        # import from the trained curve models with 3 bends
        for i in range(base_model.num_bends):
            point_index = None
            if i == 0:
                point_index = self.weight_order[min_index[0]]
            elif i == base_model.num_bends - 1:
                point_index = self.weight_order[min_index[-1]]
            elif self.fix_centers:
                point_index = self.weight_order[(min_index[0], min_index[-1])]
            if not (point_index is None):
                parameters = list(self.net.parameters())[point_index::self.num_points]
                base_parameters = list(base_model.parameters())[i::base_model.num_bends]
                for parameter, base_parameter in zip(parameters, base_parameters):
                    parameter.data.copy_(base_parameter.data) 

    def init_linear(self, init_edge_centers=True, init_triangle_center=True):
        centers_order = dict((k, v) for k, v in self.weight_order.items() \
                             if isinstance(k, tuple) and \
                             ((len(k) == 2 and init_edge_centers and (not self.fix_centers)) or \
                              (len(k) == 3 and init_triangle_center)))
        if centers_order:
            parameters = list(self.net.parameters())
            for i in range(0, len(parameters), self.num_points):
                weights = parameters[i:i+self.num_points]
                for key, index in centers_order.items():
                    value = 0
                    for m in key:
                        m_index = self.weight_order[m]
                        value += weights[m_index].data
                    weights[index].data.copy_(value / len(key))       
    ...
    def forward(self, input, t=None):
        if t is None:
            t = torch.tensor(gen_triangle_point())
        coeffs_t = self.coeff_layer(t)
        output = self.net(input, coeffs_t)
        self._compute_l2()
        return output
