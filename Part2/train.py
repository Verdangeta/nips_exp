...
from text_dataloader import Dataset_text


parser = argparse.ArgumentParser(description='DNN curve training')
parser.add_argument('--dir', type=str, default='results', metavar='DIR',
                    help='training directory (default: /tmp/curve/)')

parser.add_argument('--dataset', type=str, default='text', metavar='DATASET',
...
parser.add_argument('--model', type=str, default="GPT", metavar='MODEL', required=True,
...
parser.add_argument('--lr_final', type=float, default=None,
                    help='final learning rate (default: None)')
parser.add_argument('--m1', type=int, default=None,
                    help='hyperparameter for triangular schedule (default: None)')
...
if args.curve:
    seed1 = args.init_start.split('/')[-2].split('_')[-1]
    seed2 = args.init_end.split('/')[-2].split('_')[-1]
    args.dir = os.path.join(args.dir, '_'.join([args.model, args.dataset, seed1, seed2, 'curve', str(args.seed)]))
else:
    args.dir = os.path.join(args.dir, '_'.join([args.model, args.dataset, str(args.seed)]))
print(args.dir)
...
if args.dataset == "text":
    ...
    num_classes = loaders["train"].dataset.vocab_size
else:
...
if args.curve is None:
        model = architecture.base(num_classes, **architecture.kwargs)
else:
...
schedule = None
if args.lr and args.lr_final and args.m1 and args.m2:
    knots = [0, args.m1, args.m2]
    vals = [args.lr, args.lr, args.lr_final]
    batches_in_epoch = len(loaders['train'])
    schedule = TriangleLR(optimizer, batches_in_epoch, knots, vals)
...
for epoch in tqdm(range(start_epoch, args.epochs + 1)):
    ...
    train_res = utils.train(loaders['train'], model, optimizer, criterion, device, regularizer, lr_schedule=schedule, epoch=epoch)
